...
import sihe.exception.BusinessException;
import sihe.mapper.SettingMapper;
...
public class SettingService {
    ...
    public Setting query(Integer id) {
        ...
        query.setUserId(id);
        //注册用户时需要生成一个Setting数据,1对1,如果没有生成,业务有问题
    ...
    //默认Spring事务的传播方式为Required,当前没有事务,就创建,有就加入
    @Transactional
    public void update(Integer id, Integer batchNumber) {
        //可以使用mapper已提供单表操作方法:根据userid查询setting信息,在修改
        //第二种实习:自己写sql,一次性修改
